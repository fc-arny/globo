  ...
  class Proxy::Record
    ...
    def restrict(context, options=nil)
      if @context == context && options.nil?
        ...
        raise RuntimeError, "Perms proxies cannot be restricted with nonmatching context or options"
      end
    end
    ...
    #
    # @raise [Perms::PermissionError] when a non-whitelisted method is accessed
    # @raise [Perms::InsecureOperationError] when an insecure association is about
    #   to be fetched
    def method_missing(method, *args, &block)
      ...
      if association || pass
        ...
        elsif Perms.allow_insecure_associations
          referenced
        else
          raise Perms::InsecureOperationError,
        ...
        if [nil, '?'].include?(suffix)
          if @restrictions.allowed_fields[:view].include?(normalized_method)
            ...
            raise Perms::PermissionError, "Attempt to fetch non-whitelisted attribute #{method}"
          end
        elsif suffix == '='
    ...
    # @return [String]
    def inspect
      "#<Perms::Proxy::Record: #{@record.inspect}>"
    end
    ...
    def check_attributes
      ...
      @record.perms_validators = validators

      yield
    ensure
      @record.perms_validators = nil
    end

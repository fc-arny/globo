  ...
  class SMSC
    ...
    def send_sms(phones, message, translit = 0, time = 0, id = 0, format = 0, sender = false, query = "")
      ...
      m = _smsc_send_cmd("send", "cost=3&phones=" + _urlencode(phones) + "&mes=" + _urlencode(message) +
          ...
          (sender == false ? "" : "&sender=" + _urlencode(sender)) + "&charset=#{SMSC_CHARSET}" +
      ...
      if SMSC_DEBUG
        if m[1] > "0"
          puts "Сообщение отправлено успешно. ID: #{m[0]}, всего SMS: #{m[1]}, стоимость: #{m[2]} руб., баланс: #{m[3]} руб.\n"
        else
    ...
    def get_sms_cost(phones, message, translit = 0, format = 0, sender = false, query = "")
      ...
      if SMSC_DEBUG
        if m[1] > "0"
          puts "Стоимость рассылки: #{m[0]} руб. Всего SMS: #{m[1]}\n"
        else
          puts "Ошибка №#{m[1][1]}\n"
        end
      end
    ...
    def _smsc_send_cmd(cmd, arg = "")
      url = (SMSC_HTTPS ? "https" : "http") + "://smsc.ru/sys/#{cmd}" + ".php?login=" + _urlencode(SMSC_LOGIN) + "&psw=" + _urlencode(SMSC_PASSWORD) + "&fmt=1&#{arg}"
